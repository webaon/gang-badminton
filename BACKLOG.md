...
- [ ] **ยังไม่มีปุ่มแก้จำนวนลูกย้อนหลัง** — กรอกตอนกด "จบเกม" ครั้งเดียว
      ถ้ากรอกผิดต้องแก้ผ่าน DB ⇒ ควรมีหน้าแก้เกมที่จบแล้วก่อนปิดรอบ (WO-2.8 ใช้ตัวเลขนี้คิดเงิน)
- [ ] **`markNoShow` เขียน `session_registrations` ตรงผ่าน admin client**
      ไม่มี DB function เฉพาะเหมือน transition อื่น ⇒ ไม่มี event log และไม่มี state guard
      ระดับ DB (ตรวจใน action แทน) — ถ้า Phase ถัดไปต้องคิด penalty จาก no-show
      ควรทำเป็น DB function ให้เหมือนตัวอื่น
...
- [ ] **สัดส่วนเก็บเงินตอนยกเลิกกลางคันไม่มีที่เก็บใน policy**
      baseline [v3.3] บอก "คิดเงินบางส่วนตาม policy" แต่ schema ของ `cancellation_policy`
      (ADR-002) ไม่มีคีย์นี้ ⇒ ให้แอดมินระบุตอนกดยกเลิกแทนการแอบสร้างคีย์ policy เอง
      UI ตั้ง default ไว้ที่ 0.5 (เก็บครึ่ง) **ซึ่งเป็นตัวเลขที่เลือกเอง ไม่ได้มาจาก baseline**
      ⇒ ควรถามเจ้าของก๊วนแล้วตัดสินให้จบตอนใช้จริง
...
- [ ] **`confirmed` ที่ไม่เคยเช็คอินถูกคิดเหมือน no-show** — ตั้งใจ (กันที่ไว้แล้วไม่มา)
      แต่ถ้าแอดมินลืมเปิดคอนโซลทั้งวัน ทุกคนจะกลายเป็น "ไม่มา" แล้วโดนเก็บเงินหมด
      ⇒ ควรมีปุ่ม "เช็คอินทุกคน" หรือเตือนก่อนปิดรอบถ้าไม่มีใครเช็คอินเลย
- [ ] **ยังไม่มีหน้าสรุปยอดก่อนกดปิดรอบ** — กดแล้วสร้าง charges เลย ย้อนไม่ได้
      (`CHARGES_ALREADY_COMMITTED`) ⇒ ควรมีหน้า preview ให้ตรวจก่อน
