...
- [ ] **`waitingSince` ยังไม่มีใครคำนวณให้** — engine รับเข้ามาเป็น input
      WO-2.7 ต้องเป็นคนหาค่านี้ (เวลาที่จบเกมล่าสุด หรือเวลาเช็คอินสำหรับคนที่ยังไม่ได้เล่น)
- [ ] **ผลจัดคู่ยังไม่ถูกบันทึกลง `games`** — engine คืนแผนอย่างเดียว
      การเขียนลงตาราง + ให้แอดมินลากสลับ override เป็นงาน WO-2.7
      🔴 ตอนเขียนลง `games` ต้องเรียง `player1..4` ตามลำดับที่ engine คืนมา **ห้ามเรียงใหม่**
      ลำดับมีความหมายเรื่องทีม (ADR-003) — เรียงใหม่ = ทำลายสมดุลทีมโดยไม่รู้ตัว
